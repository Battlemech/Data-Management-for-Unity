    ...
    public class AttributeAwareExtractor : IDataMembersExtractor
    {
        ...
        private static void GetMembers(Type type, List<IDataMember> members)
        {
            ...
            members.AddRange(from field in fields
                let customAttributes = field.CustomAttributes.Select(data => data.AttributeType).ToList()
                where !customAttributes.Contains(typeof(CompilerGeneratedAttribute))
                where !customAttributes.Contains(typeof(PreventSerialization))
                select DataMember.Create(field));
